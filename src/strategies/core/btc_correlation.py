...
Tests for correlation between BTC and XMR price movements with various time lags.

Hypothesis:
- XMR may lag BTC movements due to lower liquidity and slower price discovery
- Retail-heavy market could mean delayed reactions to BTC moves
- This strategy dynamically detects optimal lag periods rather than assuming a fixed lag

Note: The existence and consistency of any lag is UNPROVEN and requires validation
with real historical data before trusting for live trading.
"""
...
class BTCCorrelationStrategy(BaseStrategy):
    def __init__(self, params: Optional[dict[str, Any]] = None):
        default_params = {
            ...
            # Lag parameters (unvalidated - tune based on real backtests)
            "expected_lag_hours": 8,  # Hypothetical average lag (NOT validated)
    ...
    def calculate_correlation(self, xmr_df: pd.DataFrame, btc_df: pd.DataFrame) -> dict[str, float]:
        ...
        return {"correlation": max_correlation, "optimal_lag": optimal_lag}
    ...
    def generate_signal(self, xmr_df: pd.DataFrame) -> Optional[Signal]:
        ...
        if btc_move and btc_move["magnitude"] >= self.params["btc_move_threshold"]:
            ...
            logger.info(
                f"BTC {btc_move['direction']} move detected: {btc_move['magnitude']:.2%} "
        ...
        same_direction = (self.last_btc_signal["direction"] == "up" and xmr_move > 0.02) or (
        ...
        # Generate signal
        signal_type = (
            SignalType.BUY if self.last_btc_signal["direction"] == "up" else SignalType.SELL
        )
        ...
        return Signal(
            ...
            metadata={
                ...
                "hours_since_btc_move": (datetime.now() - self.last_btc_signal_time).total_seconds()
                / 3600,
    ...
    def calculate_signal_strength(self, xmr_df: pd.DataFrame) -> float:
        ...
        agreeing_windows = sum(
            ...
            if (direction == "up" and move["pct_change"] > 0)
            or (direction == "down" and move["pct_change"] < 0)
        )
        multi_window_score = agreeing_windows / len(moves) if moves else 0.5

        # Combined strength
        strength = magnitude_score * 0.5 + volume_score * 0.3 + multi_window_score * 0.2
